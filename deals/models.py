...
class Deal(models.Model):
    ...
    def add_service(self, service, price=None):
        ...
        DealService.objects.create(
            deal=self, service=service, price=price or service.price
        )

    def is_expired(self):
        """Проверка просрочена ли сделка"""
        return timezone.now() > self.end_date and self.status not in [
            "completed",
            "cancelled",
            "closed",
        ]
...
class DealService(models.Model):
    ...
    service = models.ForeignKey(
        "price.Service", on_delete=models.CASCADE, verbose_name="Услуга"
    )
