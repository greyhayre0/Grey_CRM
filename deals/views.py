...
def dashboard(request):
    ...
    deals_new = (
        Deal.objects.filter(status="new")
        .select_related("client")
        .prefetch_related("services")
    )
    deals_in_progress = (
        Deal.objects.filter(status="in_progress")
        .select_related("client")
        .prefetch_related("services")
    )
    deals_ready = (
        Deal.objects.filter(status="ready")
        .select_related("client")
        .prefetch_related("services")
    )
    deals_completed = (
        Deal.objects.filter(status="completed")
        .select_related("client")
        .prefetch_related("services")
    )
    deals_cancelled = (
        Deal.objects.filter(status="cancelled")
        .select_related("client")
        .prefetch_related("services")
    )
    ...
    total_revenue = DealService.objects.aggregate(total=Sum("price"))["total"] or 0
...
def create_deal(request):
    if request.method == "POST":
        try:
            ...
            if not all([client_name, client_phone, start_date, end_date, status]):
                return JsonResponse(
                    {
                        "success": False,
                        "errors": {
                            "general": "Все обязательные полядолжны быть заполнены"
                        },
                    },
                    status=400,
                )
            ...
            for i in range(len(service_ids)):
                if service_ids[i]:  # Проверяем, что услуга выбрана
                    ...
                    DealService.objects.create(
                        deal=deal, service=service, price=price or service.price
                    )
            ...
            return JsonResponse(
                {"success": False, "errors": {"service": "Услуга не найдена"}},
                status=400,
            )
    ...
    return JsonResponse(
        {"success": False, "errors": {"general": "Неверный метод запроса"}}, status=405
    )
...
def update_deal_status(request, deal_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": "Сделка не найдена"}, status=404
        )
...
def restore_deal(request, deal_id):
    ...
    if request.method == "POST":
        try:
            deal = Deal.objects.get(id=deal_id, status__in=["completed", "cancelled"])
            deal.status = "new"
            deal.save()
