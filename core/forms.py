    ...
    class Meta:
        ...
        widgets = {
            ...
            "duration_days": forms.NumberInput(
                attrs={"placeholder": "Дней", "min": "1", "class": "form-control"}
            ),
        }
        labels = {
...
class DealForm(forms.ModelForm):
    ...
    client_name = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Введите полное имя"}
        ),
    ...
    client_phone = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "+7 (900) 123-45-67"}
        ),
        error_messages={"required": "Телефон клиента обязателен для заполнения"},
    )
    ...
    def clean_services_data(self):
        ...
        try:
            ...
            for service_data in services_list:
                if not service_data.get("service_id") or not service_data.get("price"):
                    ...
                    for service_data in services_list:
                        ...
                        if service_id and price:
                            ...
                            DealService.objects.create(
                                deal=deal, service=service, price=price
                            )

                except (json.JSONDecodeError, Service.DoesNotExist) as e:
                    raise forms.ValidationError(f"Ошибка обработки услуг: {str(e)}")
...
class ClientForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "ФИО клиента"}
            ),
