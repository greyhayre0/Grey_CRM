...
def client_detail(request, client_id):
    ...
    client = get_object_or_404(
        Client.objects.prefetch_related("deals__services", "deals__dealservice_set"),
        id=client_id,
    )
    ...
    deals = (
        ...
        .prefetch_related("services", "dealservice_set")
        .order_by("-created_at")
    )
    ...
    if request.method == "POST":
        try:
            ...
            client = Client.objects.create(
                name=name, phone=phone, email=email, notes=notes or ""
            )
